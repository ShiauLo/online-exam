<template>
  <div>
    <section class="page-hero">
      <div><span class="soft-tag">教师专属页</span><h1 class="page-hero__title">问题处理</h1><p class="page-hero__desc">接收学生问题并支持接单、转派、关闭和过程跟踪。</p></div>
      <PageToolbar>
        <template #filters>
    ...
    <section class="page-grid page-grid--two">
      <article class="surface-section glass-card">
        <SectionHeader title="问题列表" description="教师端聚焦考试类问题。" />
      ...
      <article class="surface-section glass-card">
        <SectionHeader title="处理时间线" description="展示问题处理过程。" />
...
async function loadIssues() { await runLoad(async () => { const result = await queryIssues({ pageNum: 1, pageSize: 20 }); issues.value = result.list; }, '教师问题列表加载失败，请稍后重试'); }
async function takeIssue(row: IssueRow) {
  const confirmed = await confirmAction(`确认接单处理“${row.title}”吗？`, '接单确认');
  if (!confirmed) {
    return;
  }
  await handleIssue({ issueId: row.issueId, result: '教师已接单' });
  ElMessage.success('问题已接单');
  await loadIssues();
}
async function transfer(row: IssueRow) {
  const confirmed = await confirmAction(`当前联调数据中没有第二个教师账号，是否确认查看转派提示？`, '转派说明');
  if (!confirmed) {
    return;
  }
  ElMessage.warning('当前联调环境暂无可转派的教师账号，请补充教师账号后再试');
}
async function closeCurrent(row: IssueRow) {
  const confirmed = await confirmAction(`确认关闭问题“${row.title}”吗？`, '关闭问题确认');
  if (!confirmed) {
    return;
  }
  await closeIssue({ issueId: row.issueId, comment: '教师确认问题已解决' });
