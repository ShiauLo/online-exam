  ...
  <div>
    <section class="page-hero">
      <div>
        ...
        <h1 class="page-hero__title">系统类问题</h1>
        <p class="page-hero__desc">用于处理系统问题单，支持接单、转派、关闭和查看流转时间线。</p>
      </div>
      <PageToolbar>
        <template #filters>
    ...
    <section class="page-grid page-grid--two">
      <article class="surface-section glass-card">
        <SectionHeader title="问题列表" description="聚焦系统类问题处理与闭环。" />
          ...
          <el-table-column label="操作" fixed="right" min-width="220">
            <template #default="{ row }">
              <div class="row-actions">
                <AuthorizedAction text permission="issue.handle" type="primary" @click="takeIssue(row)">接单</AuthorizedAction>
                <AuthorizedAction text permission="issue.transfer" type="warning" @click="transfer(row)">转派</AuthorizedAction>
                <AuthorizedAction text permission="issue.close" type="danger" @click="closeCurrent(row)">关闭</AuthorizedAction>
                <el-button text @click="previewIssue(row)">跟踪</el-button>
              </div>
            </template>
          </el-table-column>
        </el-table>
      ...
      <article class="surface-section glass-card">
        <SectionHeader title="处理时间线" description="展示系统问题的流转过程。" />
...
const metrics = computed(() => [
  { label: '问题总数', value: issues.value.length },
  { label: '处理中', value: issues.value.filter((item) => item.status === 'PROCESSING').length },
  { label: '已关闭', value: issues.value.filter((item) => item.status === 'CLOSED').length }
]);
async function loadIssues() { await runLoad(async () => { const result = await queryIssues({ pageNum: 1, pageSize: 20 }); issues.value = result.list; }, '运维问题列表加载失败，请稍后重试'); }
async function takeIssue(row: IssueRow) { const confirmed = await confirmAction(`确认接单处理“${row.title}”吗？`, '接单确认'); if (!confirmed) { return; } await handleIssue({ issueId: row.issueId, result: '运维已接单' }); ElMessage.success('问题已接单'); await loadIssues(); }
async function transfer(row: IssueRow) { const confirmed = await confirmAction(`当前联调数据中没有第二个运维账号，是否确认查看转派提示？`, '转派说明'); if (!confirmed) { return; } ElMessage.warning('当前联调环境暂无可转派的运维账号，请补充运维账号后再试'); }
async function closeCurrent(row: IssueRow) { const confirmed = await confirmAction(`确认关闭问题“${row.title}”吗？`, '关闭问题确认'); if (!confirmed) { return; } await closeIssue({ issueId: row.issueId, comment: '运维确认问题已解决' }); ElMessage.success('问题已关闭'); await loadIssues(); }
onMounted(async () => {
  await loadIssues();
