  ...
  <div>
    <section class="page-hero">
      <div>
        ...
        <p class="page-hero__desc">查询系统运行与备份日志，帮助运维快速排查异常。</p>
      </div>
      <PageToolbar>
        <template #filters>
    ...
    <section class="surface-section glass-card">
      <SectionHeader title="系统日志列表" description="默认只看系统类日志。" />
      <StatePanel v-if="loadError" type="error" title="系统日志加载失败" :description="loadError" action-text="重新加载" @action="loadLogs" />
      <StatePanel v-else-if="!loading && !logs.length" type="empty" title="暂无系统日志" description="当前没有匹配的系统日志记录，可调整筛选条件后重试。" />
...
const metrics = computed(() => [
  { label: '日志总数', value: logs.value.length },
  { label: '成功记录', value: logs.value.filter((item) => item.result === 'success').length },
  { label: '异常记录', value: logs.value.filter((item) => item.result !== 'success').length }
]);
async function loadLogs() { await runLoad(async () => { const result = await queryLogs({ pageNum: 1, pageSize: 20, logType: 'system', keyword: keyword.value }); logs.value = result.list; }, '系统日志加载失败，请稍后重试'); }
